...
#include "Array_lib/array.h"        //вся работа с массивом хранится тут

const char *MENU[] = {"0. Exit", "1. We working with integer numbers", "2. We working with real numbers",
                      "3. We working with complex numbers"};
...
const char *MENU_REAL_WHERE[] = {"0. Back", "1. Positive", "2. Negative"};
const int MENU_REAL_WHERE_SIZE = sizeof(MENU_REAL_WHERE) / sizeof(MENU_REAL_WHERE[0]);
...
int main (int argc, char **argv)
{
    ...
    do {
        ...
        switch (menu) {
            ...
            case 2: {
                int menu_real;
                Array array1 = constructRealArray();
                do {
                    menu_real = dialog(GENERAL_MENU, GENERAL_MENU_SIZE);
                    switch (menu_real) {
                        case 1: {
                            Try {
                                        sort(&array1);
                                        printRealArray(&array1);
                                    } Catch(EXCEPTION) {
                                handlerError(EXCEPTION);
                            }
                        }
                            ...
                            do {
                                ...
                                if (menu_map >= 1 && menu_map <= 2) {
                                    Try {
                                                Array array2;
                                                if (menu_map == 1)
                                                    array2 = map(realFuncMap1, &array1);
                                                else if (menu_map == 2)
                                                    array2 = map(realFuncMap2, &array1);
                                                printRealArray(&array2);
                                                destructArray(&array2);
                                            } Catch(EXCEPTION) {
                                        handlerError(EXCEPTION);
                                    }
                        ...
                        case 3: {
                            ...
                            do {
                                ...
                                if (menu_reduce >= 1 && menu_reduce <= 2) {
                                    ...
                                    scanf("%lf", &start_element);
                                    Try {
                                                if (menu_reduce == 1)
                                                    printf("%lf",
                                                           *((double *) reduce(realFuncReduce1, &array1,
                                                                               &start_element)));
                                                else if (menu_reduce == 2)
                                                    printf("%lf",
                                                           *((double *) reduce(realFuncReduce2, &array1,
                                                                               &start_element)));
                                                printf("\n");
                                                while (getchar() != '\n');
                                            } Catch(EXCEPTION) {
                        ...
                        case 4: {
                            int menu_where;
                            do {
                                menu_where = dialog(MENU_REAL_WHERE, MENU_REAL_WHERE_SIZE);
                                if (menu_where >= 1 && menu_where <= 2) {
                                    Try {
                                                Array array2;
                                                if (menu_where == 1)
                                                    array2 = where(realFuncWhere1, &array1);
                                                if (menu_where == 2)
                                                    array2 = where(realFuncWhere2, &array1);
                                                printRealArray(&array2);
                                                destructArray(&array2);
                                            } Catch(EXCEPTION) {
                                        handlerError(EXCEPTION);
                        ...
                        case 5: {
                            Try {
                                        Array array2 = constructRealArray();
                                        Array array3 = concatenat(&array1, &array2);
                                        printRealArray(&array3);
                                        destructArray(&array3);
                                        destructArray(&array2);
                                    } Catch(EXCEPTION) {
                                handlerError(EXCEPTION);
