...
void Graph::DepthSearch(sf::RenderWindow &window, int position, size_t search) {
    ...
    shapes.setPosition(points[0].first - vertex_raduis, points[0].second - vertex_raduis);
    window.draw(shapes);
    text.setFillColor(sf::Color::Red);
    text.setString(std::to_string(1));
    text.setOrigin(bounds.left + bounds.width / 2.0f, bounds.top + bounds.height / 2.0f);
    text.setPosition(points[0].first, points[0].second);
    window.draw(text);
    window.display();
    sf::sleep(sf::milliseconds(2000));
}

void Graph::SearchOstov(sf::RenderWindow &window, int position, int num_vertex) {
    std::pair<float, float> center(0, height_ / 2);
    if (position == 1) center.first = width_ / 6;
    else if (position == 2) center.first = width_ / 6 + width_ / 3;
    else if (position == 3) center.first = width_ / 6 + 2 * width_ / 3;
    ...
    for (int i = 0; i < matrix_.GetSize(); ++i) {
        for (int j = i; j < matrix_.GetSize(); ++j) {
            if (matrix_[i][j] != 0)
                graph_edge.push_back(Edge{static_cast<size_t>(i), static_cast<size_t>(j), matrix_[i][j]});
        }
    }
    num_edge = graph_edge.size();

    int cost = 0;
    std::vector<std::pair<int, int> > res;

    sort(graph_edge.begin(), graph_edge.end(), [](Edge &left, Edge &right) {
        return left.weight < right.weight;
    });
    std::vector<int> tree_id(num_vertex);
    for (int i = 0; i < num_vertex; ++i)
        tree_id[i] = i;
    for (int i = 0; i < num_edge; ++i) {
        int a = graph_edge[i].src, b = graph_edge[i].dst, l = graph_edge[i].weight;
    ...
    for (int i = 0; i < matrix_.GetSize(); ++i) {
        for (int j = 0; j < matrix_.GetSize(); ++j) {
            for (auto &re: res) {
                if(i == re.first && j == re.second)
                    matrix_[i][j] = 1;
            }
        }
    }
    this->drawInCircle(window, position);
    sf::sleep(sf::milliseconds(2000));
};