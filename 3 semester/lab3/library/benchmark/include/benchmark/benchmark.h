...
static void BM_memcpy(benchmark::State& state) {
  char* source = new char[state.range(0)]; char* dst = new char[state.range(0)];
  memset(source, 'x', state.range(0));
  for (auto _ : state)
    memcpy(dst, source, state.range(0));
  state.SetBytesProcessed(state.iterations() * state.range(0));
  delete[] source; delete[] dst;
}
